...
std::string key_to_wif1(const fc::ecc::private_key& key)
{
...
{/*
  ...
  genesis_state genesis_config = fc::json::from_file(fc::path("genesis_bts.json")).as<genesis_state>();
  ...
  return 0;
  */
	//fc::path p="./genesis_bts.json";
	//genesis_state genesis_config = fc::json::from_file(p).as<genesis_state>();
	//std::cout << genesis_config.delegates[0].name;
	//return 0;



	genesis_state config;
	config.delegates.reserve(10);
	config.initial_balances.reserve(10);
	std::vector<fc::ecc::private_key> keys;
	keys.reserve(10);
	
	 fc::time_point_sec time(10000000);
	...
	for (unsigned i = 0; i < 10; ++i)
	{
		...
		balan.balance = 500000000000;
		balan.raw_address = bts::blockchain::pts_address(keys[i].get_public_key());
		...
		for (unsigned i = 0; i < 10; i++)
		{ 
		wif_keys.push_back(key_to_wif1(keys[i]));
		}

		fc::json::save_to_file(wif_keys, fc::path("wif_keys.json"), true);

		fc::json::save_to_file(keys, fc::path("keys.json"), true);

		std::cout << "saving genesis.json\n";
		std::cout << "saving wif_keys.json, keys.json\n";


		//fc::path p = "init_genesis.json";
		//genesis_state genesis_config = fc::json::from_file(p).as<genesis_state>();
		//std::cout << genesis_config.delegates[0].name;
		...
		genesis_state genesis_config = fc::json::from_file(fc::path("init_genesis.json")).as<genesis_state>();
