...
static int unit_test_check_equal_number(lua_State *L)
{
    ...
    thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "check equal error happen");
    return 0;
}
...
static int unit_test_check(lua_State *L)
{
    ...
    thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "check condition error happen");
    return 0;
}
...
static int throw_error(lua_State *L)
{
    ...
    {
        thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_THROW_ERROR, "empty error");
        // thinkyoung::lua::lib::notify_lua_state_stop(L);
        return 0;
    }
    const char *msg = luaL_checkstring(L, -1);
    thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_THROW_ERROR, msg);
...
static int register_storage(lua_State *L)
{
    if (!lua_istable(L, 1))
    {
        thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "first argument of register_storage must be contract");
        return 0;
    }
    ...
    thinkyoung::lua::api::register_storage(L, contract_name, name);
    return 0;
}
...
static struct GluaStorageValue get_last_storage_changed_value(lua_State *L, const char *contract_id,
  GluaStorageChangeList *list, std::string &key)
{
...
bool luaL_commit_storage_changes(lua_State *L)
{
  ...
  if (maybe_storage_changed_contract_ids && maybe_storage_changed_contract_ids->size()>0)
  {
    ...
    {
		auto stream = thinkyoung::lua::api::open_contract_by_address(L, it->first.c_str());
		if (!stream)
		{
		...
		if (thinkyoung::lua::lib::is_calling_contract_init_api(L))
		{
			...
			if (it->first == starting_contract_address)
			{
				...
				{
					thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "some storage of this contract not init");
					return false;
				}
				for(auto &p1 : *(it->second))
				{
					...
					{
						thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "Can't find storage %s", p1.first.c_str());
						return false;
					}
						...
						|| thinkyoung::blockchain::is_any_array_storage_value_type(p1.second.after.type))
					{
						...
						{
							thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "storage %s type not matched in chain", p1.first.c_str());
							return false;
						}
					...
					&& storage_properties_in_chain.find(it2->first) != storage_properties_in_chain.end())
				{
						...
						|| thinkyoung::blockchain::is_any_array_storage_value_type(it2->second.after.type))
					{
						...
						if (!thinkyoung::blockchain::is_any_table_storage_value_type(storage_info_in_chain)
							&& !thinkyoung::blockchain::is_any_array_storage_value_type(storage_info_in_chain))
						{
							thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "storage %s type not matched in chain", it2->first.c_str());
							return false;
						}
    ...
    auto result = thinkyoung::lua::api::commit_storage_changes_to_thinkyoung(L, changes);
    if (storage_changelist_node.type == LUA_STATE_VALUE_POINTER && nullptr != storage_changelist_node.value.pointer_value)
    {
        GluaStorageChangeList *list = (GluaStorageChangeList*)storage_changelist_node.value.pointer_value;
        list->clear();
...
namespace glua {
	namespace lib {
			...
			const char *contract_id, const char *name)
		{
			...
			{
				thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "contract can only access its own storage directly");
				thinkyoung::lua::lib::notify_lua_state_stop(L);
				L->force_stopping = true;
				return 0;
			}
		...
		*/
		int thinkyounglib_get_storage(lua_State *L)
		{
			...
			if (lua_gettop(L) < 2)
			{
				thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "get_storage need 2 arguments");
				thinkyoung::lua::lib::notify_lua_state_stop(L);
				return 0;
			}
			if (!lua_istable(L, 1))
			{
				thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "first argument of get_storage must be contract");
				return 0;
			}
			lua_getfield(L, 1, "id");
			...
			{
				thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "second argument of set_storage must be string or integer");
				thinkyoung::lua::lib::notify_lua_state_stop(L);
				L->force_stopping = true;
				return 0;
			}
          ...
          const char *contract_id, const char *name, int value_index)
        {
          ...
          {
            thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "second argument of set_storage must be name");
            return 0;
          }
          ...
          {
          thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "third argument of set_storage must be value");
          return 0;
          }
          */
          ...
          {
            thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, (name_str + "storage can't change to nil").c_str());
            thinkyoung::lua::lib::notify_lua_state_stop(L);
            return 0;
          }
          ...
          {
            thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, (std::string(name) + "storage can't change type").c_str());
            thinkyoung::lua::lib::notify_lua_state_stop(L);
            return 0;
          }
            ...
            && before.type == thinkyoung::blockchain::StorageValueTypes::storage_value_null)
          {
            ...
            thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, (std::string(name) + "storage can't register storage after inited").c_str());
            thinkyoung::lua::lib::notify_lua_state_stop(L);
            return 0;
          }
          ...
          if (lua_storage_is_table(after.type))
          {
            ...
            if (nullptr != inner_table)
            {
              for (auto it = inner_table->begin(); it != inner_table->end(); ++it)
              {
                ...
                {
                  thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "storage not support nested map");
                  thinkyoung::lua::lib::notify_lua_state_stop(L);
                  return 0;
                }
              }
              ...
              for (auto it = before.value.table_value->begin(); it != before.value.table_value->end(); ++it)
              {
                ...
                if (it->second.type != thinkyoung::blockchain::StorageValueTypes::storage_value_null)
                {
                  ...
                  else
                  {
                    ...
                    {
                      thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "storage table type must be same");
                      thinkyoung::lua::lib::notify_lua_state_stop(L);
                      return 0;
                    }
                  }
		...
		*/
		int thinkyounglib_set_storage(lua_State *L)
		{
			...
			if (lua_gettop(L) < 3)
			{
				thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "set_storage need 2 arguments");
				thinkyoung::lua::lib::notify_lua_state_stop(L);
				return 0;
			}
			if (!lua_istable(L, 1))
			{
				thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "first argument of set_storage must be contract");
				return 0;
			}
			lua_getfield(L, 1, "id");
    ...
    namespace lua {
        namespace lib {
            ...
            bool lthinkyoung_set_storage(lua_State *L, const char *contract_id, const char *name, GluaStorageValue value)
            {
                ...
                return !thinkyoung::lua::api::has_exception(L);
            }
...
static int rollback_storage(lua_State *L)
{
    if (lua_gettop(L) < 1)
    {
        thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "rollback_storage need 1 arguments");
        thinkyoung::lua::lib::notify_lua_state_stop(L);
        return 0;
    }
    if (!lua_istable(L, 1))
    {
        thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "first argument of rollback_storage must be contract");
        return 0;
    }
    lua_getfield(L, 1, "name");
    auto contract_name = (const char*)luaL_checkstring(L, -1);
