...
namespace thinkyoung {
    namespace lua {
        namespace api {
            ...
            */
            bool has_exception(lua_State *L)
            {
            ...
            */
            void clear_exceptions(lua_State *L)
            {
                has_error = 0;
            }
            ...
            */
            void throw_exception(lua_State *L, int code, const char *error_format, ...)
            {
                has_error = 1;
            ...
            */
            int check_contract_api_instructions_over_limit(lua_State *L)
            {
                return 0; // FIXME: need fill by thinkyoung api
            }

            int get_stored_contract_info(lua_State *L, const char *name, std::shared_ptr<GluaContractInfo> contract_info_ret)
            {
            ...
            int get_stored_contract_info_by_address(lua_State *L, const char *address, std::shared_ptr<GluaContractInfo> contract_info_ret)
            {
                blockchain::TransactionEvaluationState* pevaluate_state = (blockchain::TransactionEvaluationState*)lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value;
            ...
            void get_contract_address_by_name(lua_State *L, const char *name, char *address, size_t *address_size)
            {
                /*
            ...
            bool check_contract_exist_by_address(lua_State *L, const char *address)
            {
                thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
                    (thinkyoung::blockchain::TransactionEvaluationState*)
                    (thinkyoung::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
            ...
            bool check_contract_exist(lua_State *L, const char *name)
            {
                /*
            ...
            std::shared_ptr<GluaModuleByteStream> get_bytestream_from_code(lua_State *L, const Code& code)
            {
            ...
            */
            std::shared_ptr<GluaModuleByteStream> open_contract(lua_State *L, const char *name)
            {
                // FXIME
                /*
                bool is_bytes = true;
            ...
            std::shared_ptr<GluaModuleByteStream> open_contract_by_address(lua_State *L, const char *address)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
                    (thinkyoung::blockchain::TransactionEvaluationState*)
            ...
            */
            ...
            GluaStorageValue get_storage_value_from_thinkyoung_by_address(lua_State *L, const char *contract_address, std::string name)
            {
                GluaStorageValue null_storage;
                null_storage.type = thinkyoung::blockchain::StorageValueTypes::storage_value_null;
                ...
                oContractStorage entry = cur_state->get_contractstorage_entry(Address::Address(std::string(contract_address), AddressType::contract_address));
                if (NOT entry.valid())
                    return null_storage;
            ...
            bool commit_storage_changes_to_thinkyoung(lua_State *L, AllContractsChangesMap &changes)
            {
                ...
                for (auto all_con_chg_iter = changes.begin(); all_con_chg_iter != changes.end(); ++all_con_chg_iter)
                {
                    ...
                    storage_op.contract_id = Address::Address(contract_id, AddressType::contract_address);
            ...
            //not use
            bool register_storage(lua_State *L, const char *contract_name, const char *name)
            {
                // TODO
            ...
            lua_Integer transfer_from_contract_to_address(lua_State *L, const char *contract_address, const char *to_address,
            ...
            lua_Integer transfer_from_contract_to_public_account(lua_State *L, const char *contract_address, const char *to_account_name,
                const char *asset_type, int64_t amount)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            int64_t get_contract_balance_amount(lua_State *L, const char *contract_address, const char* asset_symbol)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
                    ...
                    {
                        FC_CAPTURE_AND_THROW(unknown_asset_id, ("Only ALP Allowed"));
                    }
            ...
            int64_t get_transaction_fee(lua_State *L)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            uint32_t get_chain_now(lua_State *L)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            uint32_t get_chain_random(lua_State *L)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            std::string get_transaction_id(lua_State *L)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            uint32_t get_header_block_num(lua_State *L)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            uint32_t wait_for_future_random(lua_State *L, int next)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
            ...
            void emit(lua_State *L, const char* contract_id, const char* event_name, const char* event_param)
            {
              thinkyoung::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try {
                    thinkyoung::blockchain::TransactionEvaluationState* eval_state_ptr =
