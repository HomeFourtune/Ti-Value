...
#define lerror_set(L, error, error_format, ...) do {			 \
     ...
     thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, error_format, ##__VA_ARGS__);		\
} while(0)
...
#define lmalloc_error(L) do { \
		thinkyoung::lua::api::throw_exception(L, THINKYOUNG_API_SIMPLE_ERROR, "malloc memory error"); \
	} while(0)
