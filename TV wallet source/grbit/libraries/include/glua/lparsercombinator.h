...
namespace glua {
    namespace parser {
		...
		private:
			...
			bool _hidden; 
			std::string _hidden_replace_string; 
			bool _need_end_of_line; 
		public:
			inline MatchResult()
	...
	#define PARSING_MAX_LEVEL_DEPTH 100 // max recursive parser function call depth
		...
		private:
			...
			std::string _error;
			GluaParserToken _last_error_token; 
			GluaParserToken _last_second_error_token;
			GluaParserToken _last_match_token; 
		...
		public:
			...
			inline virtual int can_take_first_token(std::vector<GluaParserToken> tokens)
			{
				...
				bool end_fixed_length_token_parser = false;
